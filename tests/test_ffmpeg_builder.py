...
from facefusion.ffmpeg_builder import (
    chain,
    run,
    select_frame_range,
    set_audio_quality,
    set_audio_sample_size,
    set_stream_mode,
    set_video_quality,
)
...
def test_select_frame_range() -> None:
    ...
    assert select_frame_range(0, 100, 30) == [
        "-vf",
        "trim=start_frame=0:end_frame=100,fps=30",
    ]
...
def test_set_video_quality() -> None:
    ...
    assert set_video_quality("h264_amf", 0) == [
        "-qp_i",
        "51",
        "-qp_p",
        "51",
    ...
    assert set_video_quality("h264_amf", 50) == [
        "-qp_i",
        "26",
        "-qp_p",
        "26",
    ...
    assert set_video_quality("h264_amf", 100) == [
        "-qp_i",
        "0",
        "-qp_p",
        "0",
    ...
    assert set_video_quality("hevc_amf", 0) == [
        "-qp_i",
        "51",
        "-qp_p",
        "51",
    ...
    assert set_video_quality("hevc_amf", 50) == [
        "-qp_i",
        "26",
        "-qp_p",
        "26",
    ...
    assert set_video_quality("hevc_amf", 100) == [
        "-qp_i",
        "0",
        "-qp_p",
        "0",
