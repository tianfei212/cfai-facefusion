...
def before_all() -> None:
    ...
    subprocess.run(
        [
            ...
            get_test_example_file("source.wav"),
        ]
    )
    subprocess.run(
        [
    ...
    for output_video_format in ["avi", "m4v", "mkv", "mov", "mp4", "webm", "wmv"]:
        subprocess.run(
            [
                ...
                "-ar",
                "16000",
                get_test_example_file("target-240p-16khz." + output_video_format),
            ]
        )
...
def get_available_encoder_set() -> EncoderSet:
    if os.getenv("CI"):
        return {"audio": ["aac"], "video": ["libx264"]}
...
def test_extract_frames() -> None:
    ...
    for target_path, trim_frame_start, trim_frame_end, frame_total in test_set:
        ...
        assert (
            extract_frames(
                target_path, (452, 240), 30.0, trim_frame_start, trim_frame_end
            )
            is True
        )
...
def test_read_audio_buffer() -> None:
    assert isinstance(
        read_audio_buffer(get_test_example_file("source.mp3"), 1, 16, 1), bytes
    )
    assert isinstance(
        read_audio_buffer(get_test_example_file("source.wav"), 1, 16, 1), bytes
    )
...
def test_restore_audio() -> None:
    test_set = [
        (
            get_test_example_file("target-240p-16khz.avi"),
            get_test_output_file("target-240p-16khz.avi"),
        ),
        (
            get_test_example_file("target-240p-16khz.m4v"),
            get_test_output_file("target-240p-16khz.m4v"),
        ),
        (
            get_test_example_file("target-240p-16khz.mkv"),
            get_test_output_file("target-240p-16khz.mkv"),
        ),
        (
            get_test_example_file("target-240p-16khz.mov"),
            get_test_output_file("target-240p-16khz.mov"),
        ),
        (
            get_test_example_file("target-240p-16khz.mp4"),
            get_test_output_file("target-240p-16khz.mp4"),
        ),
        (
            get_test_example_file("target-240p-48khz.mp4"),
            get_test_output_file("target-240p-48khz.mp4"),
        ),
        (
            get_test_example_file("target-240p-16khz.webm"),
            get_test_output_file("target-240p-16khz.webm"),
        ),
        (
            get_test_example_file("target-240p-16khz.wmv"),
...
def test_replace_audio() -> None:
    ...
    for target_path, output_path in test_set:
        ...
        for output_audio_encoder in output_audio_encoders:
            ...
            assert (
                replace_audio(
                    target_path, get_test_example_file("source.mp3"), output_path
                )
                is True
            )
            assert (
                replace_audio(
                    target_path, get_test_example_file("source.wav"), output_path
                )
                is True
            )
