...
def test_get_inference_pool() -> None:
    ...
    with patch("facefusion.inference_manager.detect_app_context", return_value="cli"):
        ...
        assert isinstance(
            INFERENCE_POOL_SET.get("cli")
            .get("facefusion.content_analyser.nsfw_1.nsfw_2.nsfw_3.0.cpu")
            .get("nsfw_1"),
            InferenceSession,
        )
    ...
    assert INFERENCE_POOL_SET.get("cli").get(
        "facefusion.content_analyser.nsfw_1.nsfw_2.nsfw_3.0.cpu"
    ).get("nsfw_1") == INFERENCE_POOL_SET.get("ui").get(
        "facefusion.content_analyser.nsfw_1.nsfw_2.nsfw_3.0.cpu"
    ).get("nsfw_1")
