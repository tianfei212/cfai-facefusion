...
def before_all() -> None:
    ...
    subprocess.run(
        [
            ...
            "-vframes",
            "1",
            get_test_example_file("目标-240p.webp"),
        ]
    )
    subprocess.run(
        [
            "ffmpeg",
            "-i",
            get_test_example_file("target-1080p.mp4"),
            "-vframes",
            "1",
            get_test_example_file("target-1080p.jpg"),
        ]
    )
    subprocess.run(
        [
            ...
            "-vf",
            "hue=s=0",
            get_test_example_file("target-240p-0sat.jpg"),
        ]
    )
    subprocess.run(
        [
            ...
            "-vf",
            "transpose=0",
            get_test_example_file("target-240p-90deg.jpg"),
        ]
    )
    subprocess.run(
        [
            ...
            "-vf",
            "transpose=0",
            get_test_example_file("target-1080p-90deg.jpg"),
        ]
    )
    subprocess.run(
        [
    ...
    subprocess.run(
        [
            ...
            "-vf",
            "transpose=0",
            get_test_example_file("target-240p-90deg.mp4"),
        ]
    )
    subprocess.run(
        [
            ...
            "-vf",
            "transpose=0",
            get_test_example_file("target-1080p-90deg.mp4"),
        ]
    )
...
def test_detect_image_resolution() -> None:
    assert detect_image_resolution(get_test_example_file("target-240p.jpg")) == (
        426,
        226,
    )
    assert detect_image_resolution(get_test_example_file("target-240p-90deg.jpg")) == (
        226,
        426,
    )
    assert detect_image_resolution(get_test_example_file("target-1080p.jpg")) == (
        2048,
        1080,
    )
    assert detect_image_resolution(get_test_example_file("target-1080p-90deg.jpg")) == (
        1080,
        2048,
    )
...
def test_restrict_image_resolution() -> None:
    assert restrict_image_resolution(
        get_test_example_file("target-1080p.jpg"), (426, 226)
    ) == (426, 226)
    assert restrict_image_resolution(
        get_test_example_file("target-1080p.jpg"), (2048, 1080)
    ) == (2048, 1080)
    assert restrict_image_resolution(
        get_test_example_file("target-1080p.jpg"), (4096, 2160)
    ) == (2048, 1080)


def test_read_video_frame() -> None:
    assert hasattr(
        read_video_frame(get_test_example_file("target-240p-25fps.mp4")),
        "__array_interface__",
    )
...
def test_count_video_frame_total() -> None:
    assert (
        count_video_frame_total(get_test_example_file("target-240p-25fps.mp4")) == 270
    )
    assert (
        count_video_frame_total(get_test_example_file("target-240p-30fps.mp4")) == 324
    )
    assert (
        count_video_frame_total(get_test_example_file("target-240p-60fps.mp4")) == 648
    )
...
def test_predict_video_frame_total() -> None:
    assert (
        predict_video_frame_total(
            get_test_example_file("target-240p-25fps.mp4"), 12.5, 0, 100
        )
        == 50
    )
    assert (
        predict_video_frame_total(
            get_test_example_file("target-240p-25fps.mp4"), 25, 0, 100
        )
        == 100
    )
    assert (
        predict_video_frame_total(
            get_test_example_file("target-240p-25fps.mp4"), 25, 0, 200
        )
        == 200
    )
...
def test_count_trim_frame_total() -> None:
    assert (
        count_trim_frame_total(get_test_example_file("target-240p.mp4"), 0, 200) == 200
    )
    assert (
        count_trim_frame_total(get_test_example_file("target-240p.mp4"), 70, 270) == 200
    )
    assert (
        count_trim_frame_total(get_test_example_file("target-240p.mp4"), -10, None)
        == 270
    )
    assert (
        count_trim_frame_total(get_test_example_file("target-240p.mp4"), None, -10) == 0
    )
    assert (
        count_trim_frame_total(get_test_example_file("target-240p.mp4"), 280, None) == 0
    )
    assert (
        count_trim_frame_total(get_test_example_file("target-240p.mp4"), None, 280)
        == 270
    )
    assert (
        count_trim_frame_total(get_test_example_file("target-240p.mp4"), None, None)
        == 270
    )


def test_restrict_trim_frame() -> None:
    assert restrict_trim_frame(get_test_example_file("target-240p.mp4"), 0, 200) == (
        0,
        200,
    )
    assert restrict_trim_frame(get_test_example_file("target-240p.mp4"), 70, 270) == (
        70,
        270,
    )
    assert restrict_trim_frame(get_test_example_file("target-240p.mp4"), -10, None) == (
        0,
        270,
    )
    assert restrict_trim_frame(get_test_example_file("target-240p.mp4"), None, -10) == (
        0,
        0,
    )
    assert restrict_trim_frame(get_test_example_file("target-240p.mp4"), 280, None) == (
        270,
        270,
    )
    assert restrict_trim_frame(get_test_example_file("target-240p.mp4"), None, 280) == (
        0,
        270,
    )
    assert restrict_trim_frame(
        get_test_example_file("target-240p.mp4"), None, None
    ) == (0, 270)


def test_detect_video_resolution() -> None:
    assert detect_video_resolution(get_test_example_file("target-240p.mp4")) == (
        426,
        226,
    )
    assert detect_video_resolution(get_test_example_file("target-240p-90deg.mp4")) == (
        226,
        426,
    )
    assert detect_video_resolution(get_test_example_file("target-1080p.mp4")) == (
        2048,
        1080,
    )
    assert detect_video_resolution(get_test_example_file("target-1080p-90deg.mp4")) == (
        1080,
        2048,
    )
...
def test_restrict_video_resolution() -> None:
    assert restrict_video_resolution(
        get_test_example_file("target-1080p.mp4"), (426, 226)
    ) == (426, 226)
    assert restrict_video_resolution(
        get_test_example_file("target-1080p.mp4"), (2048, 1080)
    ) == (2048, 1080)
    assert restrict_video_resolution(
        get_test_example_file("target-1080p.mp4"), (4096, 2160)
    ) == (2048, 1080)
...
def test_calc_histogram_difference() -> None:
    ...
    assert (
        calculate_histogram_difference(source_vision_frame, source_vision_frame) == 1.0
    )
    assert (
        calculate_histogram_difference(source_vision_frame, target_vision_frame) < 0.5
    )
...
def test_match_frame_color() -> None:
    ...
    assert (
        calculate_histogram_difference(source_vision_frame, output_vision_frame) > 0.5
    )
