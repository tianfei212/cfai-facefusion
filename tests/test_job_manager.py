...
def test_find_job_ids() -> None:
    ...
    assert find_job_ids("drafted") == [
        "job-test-find-job-ids-1",
        "job-test-find-job-ids-2",
        "job-test-find-job-ids-3",
    ]
...
def test_remix_step() -> None:
    ...
    assert steps[2].get("args").get("target_path") == get_step_output_path(
        "job-test-remix-step", 0, args_1.get("output_path")
    )
    ...
    assert steps[3].get("args").get("target_path") == get_step_output_path(
        "job-test-remix-step", 2, args_2.get("output_path")
    )
