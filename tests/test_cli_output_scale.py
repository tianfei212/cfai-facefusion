...
@pytest.mark.parametrize(
    ...
    [(0.5, (212, 112)), (1.0, (426, 226)), (2.0, (852, 452)), (8.0, (3408, 1808))],
)
def test_output_image_scale(
    output_image_scale: Scale, output_image_resolution: Resolution
) -> None:
    output_file_path = get_test_output_file(
        "test-output-image-scale-" + str(output_image_scale) + ".jpg"
    )
    commands = [
        sys.executable,
        "facefusion.py",
...
def test_output_video_scale(
    output_video_scale: Scale, output_video_resolution: Resolution
) -> None:
    output_file_path = get_test_output_file(
        "test-output-video-scale-" + str(output_video_scale) + ".mp4"
    )
    commands = [
        sys.executable,
        "facefusion.py",
