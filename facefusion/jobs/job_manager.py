...
def init_jobs(jobs_path: str) -> bool:
    ...
    JOBS_PATH = jobs_path
    job_status_paths = [
        os.path.join(JOBS_PATH, job_status)
...
def delete_jobs(halt_on_error: bool) -> bool:
    job_ids = (
        find_job_ids("drafted")
        + find_job_ids("queued")
        + find_job_ids("failed")
        + find_job_ids("completed")
    )
    has_error = False
...
def validate_job(job_id: str) -> bool:
    ...
    return bool(
        job
        and "version" in job
        and "date_created" in job
        and "date_updated" in job
        and "steps" in job
    )
...
def add_step(job_id: str, step_args: Args) -> bool:
    ...
    if job:
        job.get("steps").append({"args": step_args, "status": "drafted"})
...
def insert_step(job_id: str, step_index: int, step_args: Args) -> bool:
    ...
    if job and has_step(job_id, step_index):
        job.get("steps").insert(step_index, {"args": step_args, "status": "drafted"})
        return update_job_file(job_id, job)
    return False
