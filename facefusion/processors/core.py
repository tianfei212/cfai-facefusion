...
def load_processor_module(processor: str) -> Any:
    try:
        processor_module = importlib.import_module(
            "facefusion.processors.modules." + processor
        )
        ...
        logger.error(
            wording.get("processor_not_loaded").format(processor=processor), __name__
        )
        logger.debug(exception.msg, __name__)
        hard_exit(1)
    except NotImplementedError:
        logger.error(
            wording.get("processor_not_implemented").format(processor=processor),
            __name__,
        )
        hard_exit(1)
