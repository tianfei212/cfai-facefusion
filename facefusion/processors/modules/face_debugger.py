    ...
    if group_processors:
        group_processors.add_argument(
            ...
            default=config.get_str_list(
                "processors", "face_debugger_items", "face-landmark-5/68 face-mask"
            ),
...
def draw_face_mask(target_face: Face, temp_vision_frame: VisionFrame) -> VisionFrame:
    ...
    crop_vision_frame, affine_matrix = warp_face_by_face_landmark_5(
        temp_vision_frame, face_landmark_5_68, "arcface_128", (512, 512)
    )
    ...
    if "box" in state_manager.get_item("face_mask_types"):
        box_mask = create_box_mask(
            crop_vision_frame, 0, state_manager.get_item("face_mask_padding")
        )
    ...
    if "area" in state_manager.get_item("face_mask_types"):
        face_landmark_68 = cv2.transform(
            face_landmark_68.reshape(1, -1, 2), affine_matrix
        ).reshape(-1, 2)
    ...
    inverse_contours, _ = cv2.findContours(
...
def draw_face_landmark_5(
    target_face: Face, temp_vision_frame: VisionFrame
) -> VisionFrame:
    face_landmark_5 = target_face.landmark_set.get("5")
    point_color = 0, 0, 255
