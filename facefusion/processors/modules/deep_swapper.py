...
def create_static_model_set(download_scope: DownloadScope) -> ModelSet:
    ...
    for model_scope, model_name in model_config:
        ...
        model_set[model_id] = {
            "hashes": {
                "deep_swapper": {
                    "url": resolve_download_url_by_provider(
                        "huggingface",
                        "deepfacelive-models-" + model_scope,
                        model_name + ".hash",
                    ),
                    "path": resolve_relative_path(
                        "../.assets/models/" + model_scope + "/" + model_name + ".hash"
                    ),
                }
            },
            "sources": {
                "deep_swapper": {
                    ...
                    "path": resolve_relative_path(
                        "../.assets/models/" + model_scope + "/" + model_name + ".dfm"
                    ),
                }
            },
    ...
    if custom_model_file_paths:
        for model_file_path in custom_model_file_paths:
            ...
            model_set[model_id] = {
                "sources": {
                    "deep_swapper": {"path": resolve_relative_path(model_file_path)}
                },
                "template": "dfl_whole_face",
            }
    ...
    if group_processors:
        group_processors.add_argument(
            ...
            default=config.get_str_value(
                "processors", "deep_swapper_model", "iperov/elon_musk_224"
            ),
...
def swap_face(target_face: Face, temp_vision_frame: VisionFrame) -> VisionFrame:
    ...
    crop_vision_frame, crop_source_mask, crop_target_mask = forward(
        crop_vision_frame, deep_swapper_morph
    )
    crop_vision_frame = normalize_crop_frame(crop_vision_frame)
    crop_vision_frame = conditional_match_frame_color(
        crop_vision_frame_raw, crop_vision_frame
    )
...
def forward(
    crop_vision_frame: VisionFrame, deep_swapper_morph: DeepSwapperMorph
    ...
    with thread_semaphore():
        crop_target_mask, crop_vision_frame, crop_source_mask = deep_swapper.run(
            None, deep_swapper_inputs
        )
...
def prepare_crop_frame(crop_vision_frame: VisionFrame) -> VisionFrame:
    crop_vision_frame = cv2.addWeighted(
        crop_vision_frame,
        1.75,
        cv2.GaussianBlur(crop_vision_frame, (0, 0), 2),
        -0.75,
        0,
    )
