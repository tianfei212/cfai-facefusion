    ...
    if group_processors:
        group_processors.add_argument(
            ...
            default=config.get_str_value(
                "processors", "expression_restorer_model", "live_portrait"
            ),
        ...
        group_processors.add_argument(
            ...
            type=int,
            default=config.get_int_value(
                "processors", "expression_restorer_factor", "80"
            ),
            choices=processors_choices.expression_restorer_factor_range,
            metavar=create_int_metavar(
                processors_choices.expression_restorer_factor_range
            ),
        )
        group_processors.add_argument(
            "--expression-restorer-areas",
            help=wording.get("help.expression_restorer_areas").format(
                choices=", ".join(processors_choices.expression_restorer_areas)
            ),
...
def apply_args(args: Args, apply_state_item: ApplyStateItem) -> None:
    ...
    apply_state_item(
        "expression_restorer_factor", args.get("expression_restorer_factor")
    )
...
def pre_process(mode: ProcessMode) -> bool:
    if mode == "stream":
        logger.error(
            wording.get("stream_not_supported") + wording.get("exclamation_mark"),
            __name__,
        )
        return False
    if (
...
def restore_expression(
    target_face: Face, target_vision_frame: VisionFrame, temp_vision_frame: VisionFrame
) -> VisionFrame:
    model_template = get_model_options().get("template")
    model_size = get_model_options().get("size")
    expression_restorer_factor = float(
        numpy.interp(
            float(state_manager.get_item("expression_restorer_factor")),
            [0, 100],
            [0, 1.2],
        )
    )
    target_crop_vision_frame, _ = warp_face_by_face_landmark_5(
        target_vision_frame,
        target_face.landmark_set.get("5/68"),
        model_template,
        model_size,
    )
    temp_crop_vision_frame, affine_matrix = warp_face_by_face_landmark_5(
        temp_vision_frame,
        target_face.landmark_set.get("5/68"),
        model_template,
        model_size,
    ...
    box_mask = create_box_mask(
        temp_crop_vision_frame, state_manager.get_item("face_mask_blur"), (0, 0, 0, 0)
    )
    crop_masks = [box_mask]
    ...
    temp_crop_vision_frame = apply_restore(
        target_crop_vision_frame, temp_crop_vision_frame, expression_restorer_factor
    )
    temp_crop_vision_frame = normalize_crop_frame(temp_crop_vision_frame)
    crop_mask = numpy.minimum.reduce(crop_masks).clip(0, 1)
    paste_vision_frame = paste_back(
        temp_vision_frame, temp_crop_vision_frame, crop_mask, affine_matrix
    )
    return paste_vision_frame


def apply_restore(
    target_crop_vision_frame: VisionFrame,
    temp_crop_vision_frame: VisionFrame,
    expression_restorer_factor: float,
) -> VisionFrame:
    ...
    pitch, yaw, roll, scale, translation, temp_expression, motion_points = (
        forward_extract_motion(temp_crop_vision_frame)
    )
    rotation = create_rotation(pitch, yaw, roll)
    target_expression = restrict_expression_areas(temp_expression, target_expression)
    target_expression = (
        target_expression * expression_restorer_factor
        + temp_expression * (1 - expression_restorer_factor)
    )
    target_expression = limit_expression(target_expression)
    target_motion_points = (
        scale * (motion_points @ rotation.T + target_expression) + translation
    )
    temp_motion_points = (
        scale * (motion_points @ rotation.T + temp_expression) + translation
    )
    crop_vision_frame = forward_generate_frame(
        feature_volume, target_motion_points, temp_motion_points
    )
    return crop_vision_frame


def restrict_expression_areas(
    temp_expression: LivePortraitExpression, target_expression: LivePortraitExpression
) -> LivePortraitExpression:
    expression_restorer_areas = state_manager.get_item("expression_restorer_areas")

    if "upper-face" not in expression_restorer_areas:
        target_expression[:, [1, 2, 6, 10, 11, 12, 13, 15, 16]] = temp_expression[
            :, [1, 2, 6, 10, 11, 12, 13, 15, 16]
        ]

    if "lower-face" not in expression_restorer_areas:
        target_expression[:, [3, 7, 14, 17, 18, 19, 20]] = temp_expression[
            :, [3, 7, 14, 17, 18, 19, 20]
        ]
...
def forward_generate_frame(
    ...
    temp_motion_points: LivePortraitMotionPoints,
) -> VisionFrame:
...
def process_frame(inputs: ExpressionRestorerInputs) -> VisionFrame:
    ...
    if target_faces:
        for target_face in target_faces:
            ...
            temp_vision_frame = restore_expression(
                target_face, target_vision_frame, temp_vision_frame
            )
