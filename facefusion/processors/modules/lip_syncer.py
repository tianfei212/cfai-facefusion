            ...
            "hashes": {
                "lip_syncer": {
                    ...
                    "path": resolve_relative_path(
                        "../.assets/models/wav2lip_gan_96.hash"
                    ),
                }
            },
            "sources": {
                "lip_syncer": {
                    ...
                    "path": resolve_relative_path(
                        "../.assets/models/wav2lip_gan_96.onnx"
                    ),
                }
            },
            "type": "wav2lip",
    ...
    if group_processors:
        group_processors.add_argument(
            ...
            default=config.get_str_value(
                "processors", "lip_syncer_model", "wav2lip_gan_96"
            ),
            choices=processors_choices.lip_syncer_models,
        )
        group_processors.add_argument(
            "--lip-syncer-weight",
            help=wording.get("help.lip_syncer_weight"),
            type=float,
            default=config.get_float_value("processors", "lip_syncer_weight", "0.5"),
            choices=processors_choices.lip_syncer_weight_range,
            metavar=create_float_metavar(processors_choices.lip_syncer_weight_range),
        )
...
def pre_process(mode: ProcessMode) -> bool:
    if not has_audio(state_manager.get_item("source_paths")):
        logger.error(
            wording.get("choose_audio_source") + wording.get("exclamation_mark"),
            __name__,
        )
        return False
    return True
...
def sync_lip(
    target_face: Face, source_voice_frame: AudioFrame, temp_vision_frame: VisionFrame
) -> VisionFrame:
    ...
    if model_type == "edtalk":
        lip_syncer_weight = numpy.array(
            [state_manager.get_item("lip_syncer_weight")]
        ...
        crop_vision_frame = forward_edtalk(
            source_voice_frame, crop_vision_frame, lip_syncer_weight
        )
    ...
    if model_type == "wav2lip":
        ...
        area_mask = create_area_mask(
            crop_vision_frame, face_landmark_68, ["lower-face"]
        )
        crop_masks.append(area_mask)
        bounding_box = create_bounding_box(face_landmark_68)
        area_vision_frame, area_matrix = warp_face_by_bounding_box(
            crop_vision_frame, bounding_box, model_size
        )
        ...
        crop_vision_frame = cv2.warpAffine(
            area_vision_frame,
            cv2.invertAffineTransform(area_matrix),
            (512, 512),
...
def forward_edtalk(
    ...
    with conditional_thread_semaphore():
        crop_vision_frame = lip_syncer.run(
            None,
            {
...
def forward_wav2lip(
    temp_audio_frame: AudioFrame, area_vision_frame: VisionFrame
) -> VisionFrame:
    lip_syncer = get_inference_pool().get("lip_syncer")

    with conditional_thread_semaphore():
        area_vision_frame = lip_syncer.run(
            None, {"source": temp_audio_frame, "target": area_vision_frame}
        )[0]
...
def prepare_audio_frame(temp_audio_frame: AudioFrame) -> AudioFrame:
    ...
    if model_type == "wav2lip":
        temp_audio_frame = (
            temp_audio_frame * state_manager.get_item("lip_syncer_weight") * 2.0
        )
...
def prepare_crop_frame(crop_vision_frame: VisionFrame) -> VisionFrame:
    ...
    if model_type == "edtalk":
        crop_vision_frame = cv2.resize(
            crop_vision_frame, model_size, interpolation=cv2.INTER_AREA
        )
    ...
    if model_type == "wav2lip":
        ...
        crop_vision_frame = (
            crop_vision_frame.transpose(0, 3, 1, 2).astype(numpy.float32) / 255.0
        )
...
def normalize_crop_frame(crop_vision_frame: VisionFrame) -> VisionFrame:
    ...
    if model_type == "edtalk":
        ...
        crop_vision_frame = cv2.resize(
            crop_vision_frame, (512, 512), interpolation=cv2.INTER_CUBIC
        )
...
def process_frame(inputs: LipSyncerInputs) -> VisionFrame:
    ...
    if target_faces:
        for target_face in target_faces:
            ...
            temp_vision_frame = sync_lip(
                target_face, source_voice_frame, temp_vision_frame
            )
