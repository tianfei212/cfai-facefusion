...
from facefusion.uis.ui_helper import (
    convert_int_none,
    convert_str_none,
    suggest_output_path,
)
    ...
    if job_manager.init_jobs(state_manager.get_item("jobs_path")):
        ...
        with gradio.Column(visible=is_job_manager) as JOB_MANAGER_WRAPPER:
            ...
            JOB_MANAGER_APPLY_BUTTON = gradio.Button(
                value=wording.get("uis.apply_button"), variant="primary", size="sm"
            )


def listen() -> None:
    JOB_MANAGER_JOB_ACTION_DROPDOWN.change(
        update,
    ...
    JOB_MANAGER_JOB_ID_DROPDOWN.change(
        update_step_index,
        inputs=JOB_MANAGER_JOB_ID_DROPDOWN,
        outputs=JOB_MANAGER_STEP_INDEX_DROPDOWN,
    )
    JOB_MANAGER_APPLY_BUTTON.click(
        apply,
        inputs=[
...
def remote_update(
    ...
    return (
        gradio.Row(visible=is_job_manager),
        gradio.Dropdown(value=get_first(uis_choices.job_manager_actions)),
        gradio.Textbox(value=None, visible=True),
...
def apply(
    job_action: JobManagerAction,
    created_job_id: str,
    selected_job_id: str,
    selected_step_index: int,
) -> Tuple[gradio.Dropdown, gradio.Textbox, gradio.Dropdown, gradio.Dropdown]:
    ...
    if is_directory(step_args.get("output_path")):
        step_args["output_path"] = suggest_output_path(
            step_args.get("output_path"), state_manager.get_item("target_path")
        )
    ...
    if job_action == "job-create":
        if created_job_id and job_manager.create_job(created_job_id):
            ...
            logger.info(
                wording.get("job_created").format(job_id=created_job_id), __name__
            )
            return (
                gradio.Dropdown(value="job-add-step"),
                gradio.Textbox(visible=False),
                gradio.Dropdown(
                    value=created_job_id, choices=updated_job_ids, visible=True
                ),
                gradio.Dropdown(),
            )
        else:
            logger.error(
                wording.get("job_not_created").format(job_id=created_job_id), __name__
            )
    ...
    if job_action == "job-submit":
        if selected_job_id and job_manager.submit_job(selected_job_id):
            ...
            logger.info(
                wording.get("job_submitted").format(job_id=selected_job_id), __name__
            )
            return (
                gradio.Dropdown(),
                gradio.Textbox(),
                gradio.Dropdown(
                    value=get_last(updated_job_ids),
    ...
    if job_action == "job-delete":
        if selected_job_id and job_manager.delete_job(selected_job_id):
            updated_job_ids = job_manager.find_job_ids(
                "drafted"
            ) + job_manager.find_job_ids("queued") + job_manager.find_job_ids(
                "failed"
            ) + job_manager.find_job_ids("completed") or ["none"]

            logger.info(
                wording.get("job_deleted").format(job_id=selected_job_id), __name__
            )
            return (
                gradio.Dropdown(),
                gradio.Textbox(),
            ...
            logger.error(
                wording.get("job_not_deleted").format(job_id=selected_job_id), __name__
            )
    ...
    if job_action == "job-add-step":
        if selected_job_id and job_manager.add_step(selected_job_id, step_args):
            ...
            logger.info(
                wording.get("job_step_added").format(job_id=selected_job_id), __name__
            )
            return (
                gradio.Dropdown(),
                gradio.Textbox(),
    ...
    if job_action == "job-remix-step":
        if (
            selected_job_id
            and job_manager.has_step(selected_job_id, selected_step_index)
            and job_manager.remix_step(selected_job_id, selected_step_index, step_args)
        ):
            ...
            logger.info(
                wording.get("job_remix_step_added").format(
                    job_id=selected_job_id, step_index=selected_step_index
                ),
                __name__,
            )
            return (
                ...
                gradio.Dropdown(visible=True),
                gradio.Dropdown(
                    value=get_last(updated_step_choices),
                    choices=updated_step_choices,
                    visible=True,
                ),
            )
        else:
    ...
    if job_action == "job-insert-step":
        if (
            selected_job_id
            and job_manager.has_step(selected_job_id, selected_step_index)
            and job_manager.insert_step(selected_job_id, selected_step_index, step_args)
        ):
            updated_step_choices = get_step_choices(selected_job_id) or ["none"]  # type:ignore[list-item]
    ...
    if job_action == "job-remove-step":
        if (
            selected_job_id
            and job_manager.has_step(selected_job_id, selected_step_index)
            and job_manager.remove_step(selected_job_id, selected_step_index)
        ):
            updated_step_choices = get_step_choices(selected_job_id) or ["none"]  # type:ignore[list-item]
...
def update(
    job_action: JobManagerAction, selected_job_id: str
) -> Tuple[gradio.Textbox, gradio.Dropdown, gradio.Dropdown]:
    if job_action == "job-create":
        return (
            gradio.Textbox(value=None, visible=True),
            gradio.Dropdown(visible=False),
    ...
    if job_action == "job-delete":
        ...
        updated_job_id = (
            selected_job_id
            if selected_job_id in updated_job_ids
            else get_last(updated_job_ids)
        )

        return (
            gradio.Textbox(visible=False),
            gradio.Dropdown(
                value=updated_job_id, choices=updated_job_ids, visible=True
            ),
            gradio.Dropdown(visible=False),
        )
