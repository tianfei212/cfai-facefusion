...
def render() -> None:
    ...
    has_expression_restorer = "expression_restorer" in state_manager.get_item(
        "processors"
    )
    ...
    register_ui_component(
        "expression_restorer_model_dropdown", EXPRESSION_RESTORER_MODEL_DROPDOWN
    )
    register_ui_component(
...
def listen() -> None:
    EXPRESSION_RESTORER_MODEL_DROPDOWN.change(
        update_expression_restorer_model,
        inputs=EXPRESSION_RESTORER_MODEL_DROPDOWN,
        outputs=EXPRESSION_RESTORER_MODEL_DROPDOWN,
    )
    EXPRESSION_RESTORER_FACTOR_SLIDER.release(
        update_expression_restorer_factor, inputs=EXPRESSION_RESTORER_FACTOR_SLIDER
    )
    EXPRESSION_RESTORER_AREAS_CHECKBOX_GROUP.change(
        update_expression_restorer_areas,
        inputs=EXPRESSION_RESTORER_AREAS_CHECKBOX_GROUP,
        outputs=EXPRESSION_RESTORER_AREAS_CHECKBOX_GROUP,
    )
...
def remote_update(
    processors: List[str],
) -> Tuple[gradio.Dropdown, gradio.Slider, gradio.CheckboxGroup]:
    has_expression_restorer = "expression_restorer" in processors
    return (
        gradio.Dropdown(visible=has_expression_restorer),
        gradio.Slider(visible=has_expression_restorer),
        gradio.CheckboxGroup(visible=has_expression_restorer),
    )


def update_expression_restorer_model(
...
def update_expression_restorer_factor(expression_restorer_factor: float) -> None:
    state_manager.set_item(
        "expression_restorer_factor", int(expression_restorer_factor)
    )


def update_expression_restorer_areas(
