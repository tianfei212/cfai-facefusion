...
def render() -> None:
    ...
    register_ui_component(
        "face_mask_areas_checkbox_group", FACE_MASK_AREAS_CHECKBOX_GROUP
    )
    register_ui_component(
...
def listen() -> None:
    FACE_OCCLUDER_MODEL_DROPDOWN.change(
        update_face_occluder_model, inputs=FACE_OCCLUDER_MODEL_DROPDOWN
    )
    FACE_PARSER_MODEL_DROPDOWN.change(
        update_face_parser_model, inputs=FACE_PARSER_MODEL_DROPDOWN
    )
    FACE_MASK_TYPES_CHECKBOX_GROUP.change(
        update_face_mask_types,
    ...
    FACE_MASK_AREAS_CHECKBOX_GROUP.change(
        update_face_mask_areas,
        inputs=FACE_MASK_AREAS_CHECKBOX_GROUP,
        outputs=FACE_MASK_AREAS_CHECKBOX_GROUP,
    )
    FACE_MASK_REGIONS_CHECKBOX_GROUP.change(
        update_face_mask_regions,
        inputs=FACE_MASK_REGIONS_CHECKBOX_GROUP,
...
def update_face_mask_types(
    ...
    return (
        ...
        gradio.CheckboxGroup(visible=has_area_mask),
        gradio.CheckboxGroup(visible=has_region_mask),
        gradio.Slider(visible=has_box_mask),
        gradio.Slider(visible=has_box_mask),
        gradio.Slider(visible=has_box_mask),
        gradio.Slider(visible=has_box_mask),
        gradio.Slider(visible=has_box_mask),
...
def update_face_mask_padding(
    face_mask_padding_top: float,
    face_mask_padding_right: float,
    face_mask_padding_bottom: float,
    face_mask_padding_left: float,
) -> None:
    face_mask_padding = (
        int(face_mask_padding_top),
        int(face_mask_padding_right),
        int(face_mask_padding_bottom),
