...
def render() -> None:
    ...
    FACE_SWAPPER_PIXEL_BOOST_DROPDOWN = gradio.Dropdown(
        ...
        choices=processors_choices.face_swapper_set.get(
...
def listen() -> None:
    ...
    FACE_SWAPPER_PIXEL_BOOST_DROPDOWN.change(
        update_face_swapper_pixel_boost, inputs=FACE_SWAPPER_PIXEL_BOOST_DROPDOWN
    )
    FACE_SWAPPER_WEIGHT_SLIDER.change(
        update_face_swapper_weight, inputs=FACE_SWAPPER_WEIGHT_SLIDER
    )
...
def remote_update(
    processors: List[str],
) -> Tuple[gradio.Dropdown, gradio.Dropdown, gradio.Slider]:
    has_face_swapper = "face_swapper" in processors
    return (
        gradio.Dropdown(visible=has_face_swapper),
        gradio.Dropdown(visible=has_face_swapper),
        gradio.Slider(visible=has_face_swapper),
    )
...
def update_face_swapper_model(
    ...
    if face_swapper_module.pre_check():
        ...
        return (
            gradio.Dropdown(value=state_manager.get_item("face_swapper_model")),
            gradio.Dropdown(
                value=state_manager.get_item("face_swapper_pixel_boost"),
                choices=face_swapper_pixel_boost_choices,
            ),
            gradio.Slider(visible=has_face_swapper_weight()),
        )
