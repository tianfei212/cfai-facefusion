...
def render() -> None:
    ...
    FACE_ENHANCER_WEIGHT_SLIDER = gradio.Slider(
        ...
        visible=has_face_enhancer
        and load_processor_module("face_enhancer").get_inference_pool()
        and load_processor_module("face_enhancer").has_weight_input(),
    )
...
def listen() -> None:
    FACE_ENHANCER_MODEL_DROPDOWN.change(
        ...
        outputs=[FACE_ENHANCER_MODEL_DROPDOWN, FACE_ENHANCER_WEIGHT_SLIDER],
    )
    FACE_ENHANCER_BLEND_SLIDER.release(
        update_face_enhancer_blend, inputs=FACE_ENHANCER_BLEND_SLIDER
    )
    FACE_ENHANCER_WEIGHT_SLIDER.release(
        update_face_enhancer_weight, inputs=FACE_ENHANCER_WEIGHT_SLIDER
    )
...
def remote_update(
    processors: List[str],
) -> Tuple[gradio.Dropdown, gradio.Slider, gradio.Slider]:
...
def update_face_enhancer_model(
    ...
    if face_enhancer_module.pre_check():
        return gradio.Dropdown(
            value=state_manager.get_item("face_enhancer_model")
        ), gradio.Slider(visible=face_enhancer_module.has_weight_input())
