...
def render() -> None:
    ...
    if job_manager.init_jobs(state_manager.get_item("jobs_path")):
        ...
        JOB_LIST_REFRESH_BUTTON = gradio.Button(
            value=wording.get("uis.refresh_button"), variant="primary", size="sm"
        )


def listen() -> None:
    job_list_job_status_checkbox_group = get_ui_component(
        "job_list_job_status_checkbox_group"
    )
    if job_list_job_status_checkbox_group:
        job_list_job_status_checkbox_group.change(
            update_job_dataframe,
            inputs=job_list_job_status_checkbox_group,
            outputs=JOB_LIST_JOBS_DATAFRAME,
        )
        JOB_LIST_REFRESH_BUTTON.click(
            update_job_dataframe,
            inputs=job_list_job_status_checkbox_group,
            outputs=JOB_LIST_JOBS_DATAFRAME,
        )
