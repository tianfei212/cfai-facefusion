...
def listen() -> None:
    ...
    OUTPUT_IMAGE_SCALE_SLIDER.release(
        update_output_image_scale, inputs=OUTPUT_IMAGE_SCALE_SLIDER
    )
    OUTPUT_AUDIO_ENCODER_DROPDOWN.change(
        update_output_audio_encoder, inputs=OUTPUT_AUDIO_ENCODER_DROPDOWN
    )
    OUTPUT_AUDIO_QUALITY_SLIDER.release(
        update_output_audio_quality, inputs=OUTPUT_AUDIO_QUALITY_SLIDER
    )
    OUTPUT_AUDIO_VOLUME_SLIDER.release(
        update_output_audio_volume, inputs=OUTPUT_AUDIO_VOLUME_SLIDER
    )
    ...
    OUTPUT_VIDEO_SCALE_SLIDER.release(
        update_output_video_scale, inputs=OUTPUT_VIDEO_SCALE_SLIDER
    )
        ...
        for method in ["change", "clear"]:
            getattr(ui_component, method)(
                ...
                outputs=[
                    ...
                    OUTPUT_IMAGE_SCALE_SLIDER,
                    OUTPUT_AUDIO_ENCODER_DROPDOWN,
                    OUTPUT_AUDIO_QUALITY_SLIDER,
                    OUTPUT_AUDIO_VOLUME_SLIDER,
                    OUTPUT_VIDEO_ENCODER_DROPDOWN,
                    OUTPUT_VIDEO_PRESET_DROPDOWN,
                    OUTPUT_VIDEO_QUALITY_SLIDER,
                    OUTPUT_VIDEO_SCALE_SLIDER,
...
def remote_update() -> Tuple[
    ...
    if is_video(state_manager.get_item("target_path")):
        state_manager.set_item(
            "output_video_fps", detect_video_fps(state_manager.get_item("target_path"))
        )
        return (
            gradio.Slider(visible=False),
            gradio.Slider(visible=False),
