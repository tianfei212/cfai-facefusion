...
def render() -> None:
    ...
    if (
        ...
        in facefusion.choices.face_detector_set[
            state_manager.get_item("face_detector_model")
        ]
    ):
        face_detector_size_dropdown_options["choices"] = (
            facefusion.choices.face_detector_set[
                state_manager.get_item("face_detector_model")
            ]
        )
    with gradio.Row():
...
def listen() -> None:
    ...
    FACE_DETECTOR_ANGLES_CHECKBOX_GROUP.change(
        update_face_detector_angles,
        inputs=FACE_DETECTOR_ANGLES_CHECKBOX_GROUP,
...
def update_face_detector_model(
    ...
    if face_detector.pre_check():
        ...
        state_manager.set_item(
            "face_detector_size", get_last(face_detector_size_choices)
        )
...
def update_face_detector_angles(
    face_detector_angles: Sequence[Angle],
) -> gradio.CheckboxGroup:
    face_detector_angles = (
        face_detector_angles or facefusion.choices.face_detector_angles
    )
