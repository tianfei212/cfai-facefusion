...
def listen() -> None:
    ...
    if output_image and output_video:
        INSTANT_RUNNER_START_BUTTON.click(
            start, outputs=[INSTANT_RUNNER_START_BUTTON, INSTANT_RUNNER_STOP_BUTTON]
        )
        INSTANT_RUNNER_START_BUTTON.click(
            run,
            outputs=[
        ...
        INSTANT_RUNNER_STOP_BUTTON.click(
            stop,
            outputs=[
                INSTANT_RUNNER_START_BUTTON,
                INSTANT_RUNNER_STOP_BUTTON,
    ...
    if ui_workflow_dropdown:
        ui_workflow_dropdown.change(
            remote_update, inputs=ui_workflow_dropdown, outputs=INSTANT_RUNNER_WRAPPER
        )
...
def run() -> Tuple[gradio.Button, gradio.Button, gradio.Image, gradio.Video]:
    ...
    if is_image(step_args.get("output_path")):
        return (
            ...
            gradio.Image(value=step_args.get("output_path"), visible=True),
    ...
    if is_video(step_args.get("output_path")):
        return (
            ...
            gradio.Video(value=step_args.get("output_path"), visible=True),
        )
    return (
        gradio.Button(visible=True),
        gradio.Button(visible=False),
