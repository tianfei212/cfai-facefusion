...
def listen() -> None:
    VOICE_EXTRACTOR_MODEL_DROPDOWN.change(
        update_voice_extractor_model,
        inputs=VOICE_EXTRACTOR_MODEL_DROPDOWN,
        outputs=VOICE_EXTRACTOR_MODEL_DROPDOWN,
    )

    for ui_component in get_ui_components(["target_image", "target_video"]):
        for method in ["change", "clear"]:
            getattr(ui_component, method)(
                remote_update, outputs=VOICE_EXTRACTOR_MODEL_DROPDOWN
            )
...
def update_voice_extractor_model(
