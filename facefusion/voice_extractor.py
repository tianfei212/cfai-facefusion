...
def collect_model_downloads() -> Tuple[DownloadSet, DownloadSet]:
    ...
    for voice_extractor_model in ["kim_vocal_1", "kim_vocal_2", "uvr_mdxnet"]:
        if state_manager.get_item("voice_extractor_model") == voice_extractor_model:
            model_hash_set[voice_extractor_model] = (
                model_set.get(voice_extractor_model)
                .get("hashes")
                .get("voice_extractor")
            )
            model_source_set[voice_extractor_model] = (
                model_set.get(voice_extractor_model)
                .get("sources")
                .get("voice_extractor")
            )
...
def extract_voice(temp_audio_chunk: AudioChunk) -> VoiceChunk:
    voice_extractor = get_inference_pool().get(
        state_manager.get_item("voice_extractor_model")
    )
    voice_trim_size = 3840
    voice_chunk_size = (voice_extractor.get_inputs()[0].shape[3] - 1) * 1024
    temp_audio_chunk, audio_pad_size = prepare_audio_chunk(
        temp_audio_chunk.T, voice_chunk_size, voice_trim_size
    )
    ...
    temp_audio_chunk = normalize_audio_chunk(
        temp_audio_chunk, voice_chunk_size, voice_trim_size, audio_pad_size
    )
...
def forward(temp_audio_chunk: AudioChunk) -> AudioChunk:
    ...
    with thread_semaphore():
        temp_audio_chunk = voice_extractor.run(None, {"input": temp_audio_chunk})[0]

    return temp_audio_chunk


def prepare_audio_chunk(
    temp_audio_chunk: AudioChunk, chunk_size: int, audio_trim_size: int
    ...
    temp_audio_chunk = numpy.pad(
        temp_audio_chunk, ((0, 0), (audio_trim_size, audio_trim_size + audio_pad_size))
    )
    temp_audio_chunks = []
...
def decompose_audio_chunk(
    temp_audio_chunk: AudioChunk, audio_trim_size: int
) -> AudioChunk:
    ...
    temp_audio_chunk = scipy.signal.stft(
        temp_audio_chunk,
        nperseg=audio_frame_size,
        noverlap=audio_frame_overlap,
        window=window,
    )[2]
    ...
    temp_audio_chunk = temp_audio_chunk.reshape(
        -1, 2, 2, audio_trim_size + 1, audio_bin_total
    ).reshape(-1, audio_channel_total, audio_trim_size + 1, audio_bin_total)
...
def compose_audio_chunk(
    ...
    temp_audio_chunk = numpy.pad(
        temp_audio_chunk,
        ((0, 0), (0, 0), (0, audio_trim_size + 1 - audio_frame_total), (0, 0)),
    )
    temp_audio_chunk = temp_audio_chunk.reshape(
        -1, 2, audio_trim_size + 1, audio_bin_total
    ).transpose((0, 2, 3, 1))
...
def normalize_audio_chunk(
    temp_audio_chunk: AudioChunk,
    chunk_size: int,
    audio_trim_size: int,
    audio_pad_size: int,
) -> AudioChunk:
    temp_audio_chunk = temp_audio_chunk.reshape((-1, 2, chunk_size))
    temp_audio_chunk = temp_audio_chunk[
        :, :, audio_trim_size:-audio_trim_size
    ].transpose(1, 0, 2)
