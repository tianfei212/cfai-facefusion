...
def select_frame_range(frame_start: int, frame_end: int, video_fps: Fps) -> Commands:
    ...
    if isinstance(frame_start, int):
        return [
            "-vf",
            "trim=start_frame=" + str(frame_start) + ",fps=" + str(video_fps),
        ]
...
def set_audio_quality(audio_encoder: AudioEncoder, audio_quality: int) -> Commands:
    if audio_encoder == "aac":
        audio_compression = (
            numpy.round(numpy.interp(audio_quality, [0, 100], [0.1, 2.0]), 1)
            .astype(float)
            .item()
        )
    ...
    if audio_encoder == "libmp3lame":
        audio_compression = (
            numpy.round(numpy.interp(audio_quality, [0, 100], [9, 0]))
            .astype(int)
            .item()
        )
        return ["-q:a", str(audio_compression)]
    if audio_encoder == "libopus":
        audio_bit_rate = (
            numpy.round(numpy.interp(audio_quality, [0, 100], [64, 256]))
            .astype(int)
            .item()
        )
    ...
    if audio_encoder == "libvorbis":
        audio_compression = (
            numpy.round(numpy.interp(audio_quality, [0, 100], [-1, 10]), 1)
            .astype(float)
            .item()
        )
        return ["-q:a", str(audio_compression)]
...
def set_video_quality(video_encoder: VideoEncoder, video_quality: int) -> Commands:
    if video_encoder in ["libx264", "libx264rgb", "libx265"]:
        video_compression = (
            numpy.round(numpy.interp(video_quality, [0, 100], [51, 0]))
            .astype(int)
            .item()
        )
    ...
    if video_encoder == "libvpx-vp9":
        video_compression = (
            numpy.round(numpy.interp(video_quality, [0, 100], [63, 0]))
            .astype(int)
            .item()
        )
        return ["-crf", str(video_compression)]
    ...
    if video_encoder in ["h264_videotoolbox", "hevc_videotoolbox"]:
        video_bit_rate = (
            numpy.round(numpy.interp(video_quality, [0, 100], [1024, 50512]))
            .astype(int)
            .item()
        )
...
def set_video_preset(
    video_encoder: VideoEncoder, video_preset: VideoPreset
) -> Commands:
    if video_encoder in ["libx264", "libx264rgb", "libx265"]:
