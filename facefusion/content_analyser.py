...
def collect_model_downloads() -> Tuple[DownloadSet, DownloadSet]:
    ...
    for content_analyser_model in ["nsfw_1", "nsfw_2", "nsfw_3"]:
        model_hash_set[content_analyser_model] = (
            model_set.get(content_analyser_model).get("hashes").get("content_analyser")
        )
        model_source_set[content_analyser_model] = (
            model_set.get(content_analyser_model).get("sources").get("content_analyser")
        )
...
def prepare_detect_frame(
    temp_vision_frame: VisionFrame, model_name: str
) -> VisionFrame:
