...
def run() -> Generator[List[BenchmarkCycleSet], None, None]:
    ...
    state_manager.init_item(
        "source_paths", [".assets/examples/source.jpg", ".assets/examples/source.mp3"]
    )
    ...
    benchmarks = []
    target_paths = [
        facefusion.choices.benchmark_set.get(benchmark_resolution)
        for benchmark_resolution in benchmark_resolutions
        if benchmark_resolution in facefusion.choices.benchmark_set
    ]

    for target_path in target_paths:
        state_manager.init_item("target_path", target_path)
        state_manager.init_item(
            "output_path", suggest_output_path(state_manager.get_item("target_path"))
        )
...
def suggest_output_path(target_path: str) -> str:
    ...
    return os.path.join(
        tempfile.gettempdir(), hashlib.sha1().hexdigest()[:8] + target_file_extension
    )
