        ...
        "yunet": {
            "hashes": {
                "yunet": {
                    ...
                    "path": resolve_relative_path(
                        "../.assets/models/yunet_2023_mar.hash"
                    ),
                }
            },
            "sources": {
                "yunet": {
                    ...
                    "path": resolve_relative_path(
                        "../.assets/models/yunet_2023_mar.onnx"
                    ),
                }
            },
        },
...
def collect_model_downloads() -> Tuple[DownloadSet, DownloadSet]:
    ...
    for face_detector_model in ["retinaface", "scrfd", "yolo_face", "yunet"]:
        if state_manager.get_item("face_detector_model") in [
            "many",
            face_detector_model,
        ]:
            model_hash_set[face_detector_model] = (
                model_set.get(face_detector_model)
                .get("hashes")
                .get(face_detector_model)
            )
            model_source_set[face_detector_model] = (
                model_set.get(face_detector_model)
                .get("sources")
                .get(face_detector_model)
            )
...
def detect_faces(
    ...
    if state_manager.get_item("face_detector_model") in ["many", "yolo_face"]:
        bounding_boxes, face_scores, face_landmarks_5 = detect_with_yolo_face(
            vision_frame, state_manager.get_item("face_detector_size")
        )
        all_bounding_boxes.extend(bounding_boxes)
        all_face_scores.extend(face_scores)
    ...
    if state_manager.get_item("face_detector_model") == "yunet":
        bounding_boxes, face_scores, face_landmarks_5 = detect_with_yunet(
            vision_frame, state_manager.get_item("face_detector_size")
        )
        all_bounding_boxes.extend(bounding_boxes)
        all_face_scores.extend(face_scores)
...
def detect_faces_by_angle(
    vision_frame: VisionFrame, face_angle: Angle
) -> Tuple[List[BoundingBox], List[Score], List[FaceLandmark5]]:
    rotation_matrix, rotation_size = create_rotation_matrix_and_size(
        face_angle, vision_frame.shape[:2][::-1]
    )
    ...
    bounding_boxes = [
        transform_bounding_box(bounding_box, rotation_inverse_matrix)
        for bounding_box in bounding_boxes
    ]
    face_landmarks_5 = [
        transform_points(face_landmark_5, rotation_inverse_matrix)
...
def detect_with_retinaface(
    ...
    temp_vision_frame = restrict_frame(
    ...
    for index, feature_stride in enumerate(feature_strides):
        ...
        if numpy.any(keep_indices):
            ...
            face_landmarks_5_raw = (
                detection[index + feature_map_channel * 2] * feature_stride
            )

            for bounding_box_raw in distance_to_bounding_box(
                anchors, bounding_boxes_raw
            )[keep_indices]:
            ...
            for face_landmark_raw_5 in distance_to_face_landmark_5(
                anchors, face_landmarks_5_raw
            )[keep_indices]:
                face_landmarks_5.append(
                    face_landmark_raw_5 * [ratio_width, ratio_height]
                )
...
def detect_with_yolo_face(
    ...
    bounding_boxes_raw, face_scores_raw, face_landmarks_5_raw = numpy.split(
        detection, [4, 5], axis=1
    )
    keep_indices = numpy.where(face_scores_raw > face_detector_score)[0]

    if numpy.any(keep_indices):
        bounding_boxes_raw, face_scores_raw, face_landmarks_5_raw = (
            bounding_boxes_raw[keep_indices],
            face_scores_raw[keep_indices],
            face_landmarks_5_raw[keep_indices],
        )
        ...
        for face_landmark_raw_5 in face_landmarks_5_raw:
            face_landmarks_5.append(
                numpy.array(face_landmark_raw_5.reshape(-1, 3)[:, :2])
            )
...
def detect_with_yunet(
    ...
    for index, feature_stride in enumerate(feature_strides):
        face_scores_raw = (
            detection[index] * detection[index + feature_map_channel]
        ).reshape(-1)
        keep_indices = numpy.where(face_scores_raw >= face_detector_score)[0]
        ...
        if numpy.any(keep_indices):
            ...
            bounding_boxes_center = (
                detection[index + feature_map_channel * 2].squeeze(0)[:, :2]
                * feature_stride
                + anchors
            )
            bounding_boxes_size = (
                numpy.exp(detection[index + feature_map_channel * 2].squeeze(0)[:, 2:4])
                * feature_stride
            )
...
def normalize_detect_frame(
    detect_vision_frame: VisionFrame, normalize_range: Sequence[int]
) -> VisionFrame:
