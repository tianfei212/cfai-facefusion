...
def run_ffmpeg_with_progress(
    commands: Commands, update_progress: UpdateProgress
...
def get_available_encoder_set() -> EncoderSet:
    available_encoder_set: EncoderSet = {"audio": [], "video": []}
    commands = ffmpeg_builder.chain(ffmpeg_builder.get_encoders())
...
def extract_frames(
    target_path: str,
    temp_video_resolution: Resolution,
    temp_video_fps: Fps,
    trim_frame_start: int,
    trim_frame_end: int,
) -> bool:
    extract_frame_total = predict_video_frame_total(
        target_path, temp_video_fps, trim_frame_start, trim_frame_end
    )
    ...
    commands = ffmpeg_builder.chain(
        ...
        ffmpeg_builder.select_frame_range(
            trim_frame_start, trim_frame_end, temp_video_fps
        ),
    ...
    with tqdm(
        total=extract_frame_total,
        desc=wording.get("extracting"),
        unit="frame",
        ascii=" =",
...
def finalize_image(
    target_path: str, output_path: str, output_image_resolution: Resolution
) -> bool:
...
def restore_audio(
    target_path: str, output_path: str, trim_frame_start: int, trim_frame_end: int
) -> bool:
    ...
    commands = ffmpeg_builder.chain(
        ...
        ffmpeg_builder.select_media_range(
            trim_frame_start, trim_frame_end, target_video_fps
        ),
        ffmpeg_builder.set_input(target_path),
...
def merge_video(
    ...
    merge_frame_total = predict_video_frame_total(
        target_path, output_video_fps, trim_frame_start, trim_frame_end
    )
    temp_video_path = get_temp_file_path(target_path)
    temp_video_format = cast(VideoFormat, get_file_format(temp_video_path))
    temp_frames_pattern = get_temp_frames_pattern(target_path, "%08d")
...
def concat_video(output_path: str, temp_output_paths: List[str]) -> bool:
    ...
    with open(concat_video_path, "w") as concat_video_file:
        for temp_output_path in temp_output_paths:
            concat_video_file.write(
                "file '" + os.path.abspath(temp_output_path) + "'" + os.linesep
            )
...
def fix_audio_encoder(
    video_format: VideoFormat, audio_encoder: AudioEncoder
) -> AudioEncoder:
...
def fix_video_encoder(
    video_format: VideoFormat, video_encoder: VideoEncoder
) -> VideoEncoder:
    if video_format in ["m4v", "wmv"]:
        return "libx264"
