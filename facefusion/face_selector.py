...
def select_faces(
    reference_vision_frame: VisionFrame, target_vision_frame: VisionFrame
) -> List[Face]:
...
def find_match_faces(
    reference_faces: List[Face], target_faces: List[Face], face_distance: float
) -> List[Face]:
...
def sort_and_filter_faces(faces: List[Face]) -> List[Face]:
    if faces:
        if state_manager.get_item("face_selector_order"):
            faces = sort_faces_by_order(
                faces, state_manager.get_item("face_selector_order")
            )
        if state_manager.get_item("face_selector_gender"):
            faces = filter_faces_by_gender(
                faces, state_manager.get_item("face_selector_gender")
            )
        if state_manager.get_item("face_selector_race"):
            faces = filter_faces_by_race(
                faces, state_manager.get_item("face_selector_race")
            )
        if state_manager.get_item("face_selector_age_start") or state_manager.get_item(
            "face_selector_age_end"
        ):
            faces = filter_faces_by_age(
                faces,
...
def get_bounding_box_area(face: Face) -> float:
    return (face.bounding_box[2] - face.bounding_box[0]) * (
...
def filter_faces_by_age(
    faces: List[Face], face_selector_age_start: int, face_selector_age_end: int
) -> List[Face]:
    filter_faces = []
