...
def read_audio(audio_path: str, fps: Fps) -> Optional[List[AudioFrame]]:
    ...
    if is_audio(audio_path):
        audio_buffer = read_audio_buffer(
            audio_path, audio_sample_rate, audio_sample_size, audio_channel_total
        )
...
def read_voice(audio_path: str, fps: Fps) -> Optional[List[AudioFrame]]:
    ...
    if is_audio(audio_path):
        audio_buffer = read_audio_buffer(
            audio_path, voice_sample_rate, voice_sample_size, voice_channel_total
        )
        audio = numpy.frombuffer(audio_buffer, dtype=numpy.int16).reshape(-1, 2)
...
def create_mel_filter_bank() -> MelFilterBank:
    ...
    mel_frequency_range = numpy.linspace(
        convert_hertz_to_mel(audio_frequency_min),
        convert_hertz_to_mel(audio_frequency_max),
        mel_filter_total + 2,
    )
    indices = numpy.floor(
        (mel_bin_total + 1)
        * convert_mel_to_hertz(mel_frequency_range)
        / audio_sample_rate
    ).astype(numpy.int16)
