...
def route_job_runner() -> ErrorCode:
    if state_manager.get_item("command") == "job-run":
        ...
        if job_runner.run_job(state_manager.get_item("job_id"), process_step):
            logger.info(
                wording.get("processing_job_succeeded").format(
                    job_id=state_manager.get_item("job_id")
                ),
                __name__,
            )
...
def process_image(start_time: float) -> ErrorCode:
    ...
    logger.info(
        wording.get("copying_image").format(
    ...
    logger.info(
        wording.get("finalizing_image").format(
            resolution=pack_resolution(output_image_resolution)
        ),
        __name__,
    )
    if finalize_image(
    ...
    if is_image(state_manager.get_item("output_path")):
        logger.info(
            wording.get("processing_image_succeeded").format(
...
def process_video(start_time: float) -> ErrorCode:
    ...
    output_video_resolution = scale_resolution(
        detect_video_resolution(state_manager.get_item("target_path")),
        state_manager.get_item("output_video_scale"),
    )
    temp_video_resolution = restrict_video_resolution(
        state_manager.get_item("target_path"), output_video_resolution
    )
    ...
    logger.info(
        wording.get("extracting_frames").format(
            resolution=pack_resolution(temp_video_resolution), fps=temp_video_fps
        ),
        __name__,
    )
    ...
    logger.info(
        wording.get("merging_video").format(
            resolution=pack_resolution(output_video_resolution),
    ...
    if is_video(state_manager.get_item("output_path")):
        logger.info(
            wording.get("processing_video_succeeded").format(
                seconds=calculate_end_time(start_time)
            ),
            __name__,
        )
