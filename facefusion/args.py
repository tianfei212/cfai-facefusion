...
def reduce_job_args(args: Args) -> Args:
    job_args = {key: args[key] for key in args if key in job_store.get_job_keys()}
    return job_args


def collect_step_args() -> Args:
    step_args = {
        key: state_manager.get_item(key)
        for key in job_store.get_step_keys()  # type:ignore[arg-type]
    }
    return step_args
...
def collect_job_args() -> Args:
    job_args = {
        key: state_manager.get_item(key)
        for key in job_store.get_job_keys()  # type:ignore[arg-type]
    }
    return job_args
...
def apply_args(args: Args, apply_state_item: ApplyStateItem) -> None:
    ...
    apply_state_item(
        "face_mask_padding", normalize_padding(args.get("face_mask_padding"))
    )
    # voice extractor
    ...
    if args.get("output_video_fps") or is_video(args.get("target_path")):
        output_video_fps = normalize_fps(
            args.get("output_video_fps")
        ) or detect_video_fps(args.get("target_path"))
