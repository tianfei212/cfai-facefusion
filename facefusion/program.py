...
def create_config_path_program() -> ArgumentParser:
    ...
    group_paths.add_argument(
        "--config-path", help=wording.get("help.config_path"), default="facefusion.ini"
    )
...
def create_temp_path_program() -> ArgumentParser:
    ...
    group_paths.add_argument(
        "--temp-path",
        help=wording.get("help.temp_path"),
...
def create_jobs_path_program() -> ArgumentParser:
    ...
    group_paths.add_argument(
        "--jobs-path",
        help=wording.get("help.jobs_path"),
...
def create_source_paths_program() -> ArgumentParser:
    ...
    group_paths.add_argument(
        "-s",
        "--source-paths",
        help=wording.get("help.source_paths"),
...
def create_target_path_program() -> ArgumentParser:
    ...
    group_paths.add_argument(
        "-t",
        "--target-path",
        help=wording.get("help.target_path"),
...
def create_output_path_program() -> ArgumentParser:
    ...
    group_paths.add_argument(
        "-o",
        "--output-path",
        help=wording.get("help.output_path"),
...
def create_source_pattern_program() -> ArgumentParser:
    ...
    group_patterns.add_argument(
        "-s",
        "--source-pattern",
        help=wording.get("help.source_pattern"),
...
def create_target_pattern_program() -> ArgumentParser:
    ...
    group_patterns.add_argument(
        "-t",
        "--target-pattern",
        help=wording.get("help.target_pattern"),
...
def create_output_pattern_program() -> ArgumentParser:
    ...
    group_patterns.add_argument(
        "-o",
        "--output-pattern",
        help=wording.get("help.output_pattern"),
...
def create_face_detector_program() -> ArgumentParser:
    ...
    group_face_detector.add_argument(
        ...
        default=config.get_str_value(
            "face_detector", "face_detector_model", "yolo_face"
        ),
        choices=facefusion.choices.face_detector_models,
    )
    known_args, _ = program.parse_known_args()
    face_detector_size_choices = facefusion.choices.face_detector_set.get(
        known_args.face_detector_model
    )
    group_face_detector.add_argument(
        "--face-detector-size",
        help=wording.get("help.face_detector_size"),
        default=config.get_str_value(
            "face_detector", "face_detector_size", get_last(face_detector_size_choices)
        ),
        choices=face_detector_size_choices,
    )
    group_face_detector.add_argument(
        "--face-detector-angles",
        help=wording.get("help.face_detector_angles"),
        type=int,
...
def create_face_landmarker_program() -> ArgumentParser:
    ...
    group_face_landmarker.add_argument(
        ...
        help=wording.get("help.face_landmarker_model"),
        default=config.get_str_value(
            "face_landmarker", "face_landmarker_model", "2dfan4"
        ),
    ...
    group_face_landmarker.add_argument(
        ...
        default=config.get_float_value(
            "face_landmarker", "face_landmarker_score", "0.5"
        ),
...
def create_face_selector_program() -> ArgumentParser:
    ...
    group_face_selector.add_argument(
        ...
        default=config.get_str_value(
            "face_selector", "face_selector_mode", "reference"
        ),
        choices=facefusion.choices.face_selector_modes,
    )
    group_face_selector.add_argument(
        "--face-selector-order",
        help=wording.get("help.face_selector_order"),
        default=config.get_str_value(
            "face_selector", "face_selector_order", "large-small"
        ),
        choices=facefusion.choices.face_selector_orders,
    )
    group_face_selector.add_argument(
        "--face-selector-age-start",
        help=wording.get("help.face_selector_age_start"),
        type=int,
    ...
    group_face_selector.add_argument(
        ...
        help=wording.get("help.face_selector_age_end"),
        type=int,
    ...
    group_face_selector.add_argument(
        ...
        type=float,
        default=config.get_float_value(
            "face_selector", "reference_face_distance", "0.3"
        ),
...
def create_face_masker_program() -> ArgumentParser:
    ...
    group_face_masker.add_argument(
        "--face-occluder-model",
        help=wording.get("help.face_occluder_model"),
        default=config.get_str_value("face_masker", "face_occluder_model", "xseg_1"),
        choices=facefusion.choices.face_occluder_models,
    )
    group_face_masker.add_argument(
        "--face-parser-model",
        help=wording.get("help.face_parser_model"),
        default=config.get_str_value(
            "face_masker", "face_parser_model", "bisenet_resnet_34"
        ),
    ...
    group_face_masker.add_argument(
        "--face-mask-areas",
        help=wording.get("help.face_mask_areas").format(
            choices=", ".join(facefusion.choices.face_mask_areas)
        ),
        default=config.get_str_list(
            "face_masker",
            "face_mask_areas",
            " ".join(facefusion.choices.face_mask_areas),
        ),
...
def create_voice_extractor_program() -> ArgumentParser:
    ...
    group_voice_extractor.add_argument(
        ...
        help=wording.get("help.voice_extractor_model"),
        default=config.get_str_value(
            "voice_extractor", "voice_extractor_model", "kim_vocal_2"
        ),
...
def create_output_creation_program() -> ArgumentParser:
    ...
    group_output_creation.add_argument(
        "--output-image-scale",
        help=wording.get("help.output_image_scale"),
        type=float,
    ...
    group_output_creation.add_argument(
        ...
        help=wording.get("help.output_audio_encoder"),
        default=config.get_str_value(
            "output_creation",
    ...
    group_output_creation.add_argument(
        ...
        help=wording.get("help.output_audio_quality"),
        type=int,
    ...
    group_output_creation.add_argument(
        "--output-audio-volume",
        help=wording.get("help.output_audio_volume"),
        type=int,
    ...
    group_output_creation.add_argument(
        ...
        default=config.get_str_value(
            "output_creation", "output_video_preset", "veryfast"
        ),
    ...
    group_output_creation.add_argument(
        "--output-video-scale",
        help=wording.get("help.output_video_scale"),
        type=float,
...
def create_processors_program() -> ArgumentParser:
    ...
    group_processors.add_argument(
        "--processors",
        help=wording.get("help.processors").format(
            choices=", ".join(available_processors)
        ),
...
def create_uis_program() -> ArgumentParser:
    ...
    available_ui_layouts = [
        get_file_name(file_path)
        for file_path in resolve_file_paths("facefusion/uis/layouts")
    ]
    group_uis = program.add_argument_group("uis")
    group_uis.add_argument(
        "--open-browser",
        help=wording.get("help.open_browser"),
        action="store_true",
...
def create_download_providers_program() -> ArgumentParser:
    ...
    group_download.add_argument(
        "--download-providers",
        help=wording.get("help.download_providers").format(
            choices=", ".join(facefusion.choices.download_providers)
        ),
        default=config.get_str_list(
            "download",
            "download_providers",
            " ".join(facefusion.choices.download_providers),
        ),
...
def create_download_scope_program() -> ArgumentParser:
    ...
    group_download.add_argument(
        "--download-scope",
        help=wording.get("help.download_scope"),
...
def create_benchmark_program() -> ArgumentParser:
    ...
    group_benchmark.add_argument(
        "--benchmark-mode",
        help=wording.get("help.benchmark_mode"),
    ...
    group_benchmark.add_argument(
        ...
        help=wording.get("help.benchmark_resolutions"),
        default=config.get_str_list(
            "benchmark",
            "benchmark_resolutions",
            get_first(facefusion.choices.benchmark_resolutions),
        ),
    ...
    group_benchmark.add_argument(
        ...
        help=wording.get("help.benchmark_cycle_count"),
        type=int,
...
def create_execution_program() -> ArgumentParser:
    ...
    group_execution.add_argument(
        ...
        help=wording.get("help.execution_device_ids"),
    ...
    group_execution.add_argument(
        ...
        help=wording.get("help.execution_providers").format(
            choices=", ".join(available_execution_providers)
        ),
        default=config.get_str_list(
...
def create_halt_on_error_program() -> ArgumentParser:
    ...
    group_misc.add_argument(
        "--halt-on-error",
        help=wording.get("help.halt_on_error"),
        action="store_true",
...
def create_job_status_program() -> ArgumentParser:
    ...
    program.add_argument(
        "job_status",
        help=wording.get("help.job_status"),
...
def create_program() -> ArgumentParser:
    program = ArgumentParser(
        formatter_class=create_help_formatter_large, add_help=False
    )
    ...
    sub_program.add_parser(
        "benchmark",
        help=wording.get("help.benchmark"),
        parents=[
            create_temp_path_program(),
            collect_step_program(),
            create_benchmark_program(),
    ...
    return ArgumentParser(
        parents=[program], formatter_class=create_help_formatter_small
    )
