...
ONNXRUNTIME_SET = {"default": ("onnxruntime", "1.22.1")}
...
def cli() -> None:
    ...
    program = ArgumentParser(
        formatter_class=partial(HelpFormatter, max_help_position=50)
    )
    program.add_argument(
        "--onnxruntime",
        help=wording.get("help.install_dependency").format(dependency="onnxruntime"),
        choices=ONNXRUNTIME_SET.keys(),
        required=True,
    )
    program.add_argument(
...
def run(program: ArgumentParser) -> None:
    ...
    with open("requirements.txt") as file:
        for line in file.readlines():
            ...
            if not __line__.startswith("onnxruntime"):
                subprocess.call(
                    [shutil.which("pip"), "install", line, "--force-reinstall"]
                )
    ...
    if args.onnxruntime == "rocm":
        ...
        if python_id in ["cp310", "cp312"]:
            ...
            wheel_url = (
                "https://repo.radeon.com/rocm/manylinux/rocm-rel-6.4/" + wheel_name
            )
            subprocess.call(
                [shutil.which("pip"), "install", wheel_url, "--force-reinstall"]
            )
    else:
        subprocess.call(
            [
    ...
    if args.onnxruntime == "directml":
        subprocess.call(
            [shutil.which("pip"), "install", "numpy==1.26.4", "--force-reinstall"]
        )
