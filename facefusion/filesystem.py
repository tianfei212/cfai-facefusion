...
def same_file_extension(first_file_path: str, second_file_path: str) -> bool:
    ...
    if first_file_extension and second_file_extension:
        return get_file_extension(first_file_path) == get_file_extension(
            second_file_path
        )
    return False
...
def is_audio(audio_path: str) -> bool:
    return (
        is_file(audio_path)
        and get_file_format(audio_path) in facefusion.choices.audio_formats
    )
...
def is_image(image_path: str) -> bool:
    return (
        is_file(image_path)
        and get_file_format(image_path) in facefusion.choices.image_formats
    )
...
def is_video(video_path: str) -> bool:
    return (
        is_file(video_path)
        and get_file_format(video_path) in facefusion.choices.video_formats
    )
