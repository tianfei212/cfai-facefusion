...
def predict_video_frame_total(
    video_path: str, fps: Fps, trim_frame_start: int, trim_frame_end: int
) -> int:
    if is_video(video_path):
        video_fps = detect_video_fps(video_path)
        extract_frame_total = (
            count_trim_frame_total(video_path, trim_frame_start, trim_frame_end)
            * fps
            / video_fps
        )
...
def count_trim_frame_total(
    video_path: str, trim_frame_start: Optional[int], trim_frame_end: Optional[int]
) -> int:
    trim_frame_start, trim_frame_end = restrict_trim_frame(
        video_path, trim_frame_start, trim_frame_end
    )

    return trim_frame_end - trim_frame_start


def restrict_trim_frame(
    video_path: str, trim_frame_start: Optional[int], trim_frame_end: Optional[int]
) -> Tuple[int, int]:
    video_frame_total = count_video_frame_total(video_path)
...
def fit_contain_frame(vision_frame: VisionFrame, resolution: Resolution) -> VisionFrame:
    ...
    temp_vision_frame = numpy.pad(
        temp_vision_frame, ((start_y, end_y), (start_x, end_x), (0, 0))
    )
...
def blend_frame(
    ...
    blend_vision_frame = cv2.addWeighted(
        source_vision_frame, 1 - blend_factor, target_vision_frame, blend_factor, 0
    )
    return blend_vision_frame


def conditional_match_frame_color(
    source_vision_frame: VisionFrame, target_vision_frame: VisionFrame
) -> VisionFrame:
    histogram_factor = calculate_histogram_difference(
        source_vision_frame, target_vision_frame
    )
    target_vision_frame = blend_frame(
        target_vision_frame,
        match_frame_color(source_vision_frame, target_vision_frame),
        histogram_factor,
    )
    return target_vision_frame


def match_frame_color(
    source_vision_frame: VisionFrame, target_vision_frame: VisionFrame
) -> VisionFrame:
    color_difference_sizes = numpy.linspace(
        16, target_vision_frame.shape[0], 3, endpoint=False
    )
    ...
    for color_difference_size in color_difference_sizes:
        source_vision_frame = equalize_frame_color(
            source_vision_frame,
            target_vision_frame,
            normalize_resolution((color_difference_size, color_difference_size)),
        )
...
def equalize_frame_color(
    source_vision_frame: VisionFrame, target_vision_frame: VisionFrame, size: Size
) -> VisionFrame:
    source_frame_resize = cv2.resize(
        source_vision_frame, size, interpolation=cv2.INTER_AREA
    ).astype(numpy.float32)
    target_frame_resize = cv2.resize(
        target_vision_frame, size, interpolation=cv2.INTER_AREA
    ).astype(numpy.float32)
    color_difference_vision_frame = numpy.subtract(
        source_frame_resize, target_frame_resize
    )
    color_difference_vision_frame = cv2.resize(
        color_difference_vision_frame,
        target_vision_frame.shape[:2][::-1],
        interpolation=cv2.INTER_CUBIC,
    )
    target_vision_frame = (
        numpy.add(target_vision_frame, color_difference_vision_frame)
        .clip(0, 255)
        .astype(numpy.uint8)
    )
    return target_vision_frame
...
def calculate_histogram_difference(
    ...
    histogram_source = cv2.calcHist(
        [cv2.cvtColor(source_vision_frame, cv2.COLOR_BGR2HSV)],
        [0, 1],
        None,
        [50, 60],
        [0, 180, 0, 256],
    ...
    histogram_target = cv2.calcHist(
        [cv2.cvtColor(target_vision_frame, cv2.COLOR_BGR2HSV)],
        [0, 1],
        None,
        [50, 60],
        [0, 180, 0, 256],
...
def create_tile_frames(
    ...
    pad_size_bottom = (
        pad_size_top + tile_width - (vision_frame.shape[0] + 2 * size[1]) % tile_width
    )
    pad_size_right = (
        pad_size_top + tile_width - (vision_frame.shape[1] + 2 * size[1]) % tile_width
    )
    pad_vision_frame = numpy.pad(
        vision_frame,
        ((pad_size_top, pad_size_bottom), (pad_size_top, pad_size_right), (0, 0)),
    )
