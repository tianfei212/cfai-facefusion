...
def get_available_execution_providers() -> List[ExecutionProvider]:
    ...
    for (
        execution_provider,
        execution_provider_value,
    ) in facefusion.choices.execution_provider_set.items():
...
def create_inference_session_providers(
    execution_device_id: str, execution_providers: List[ExecutionProvider]
) -> List[InferenceSessionProvider]:
    inference_session_providers: List[InferenceSessionProvider] = []

    for execution_provider in execution_providers:
        if execution_provider == "cuda":
            inference_session_providers.append(
                (
                    facefusion.choices.execution_provider_set.get(execution_provider),
                    {
        ...
        if execution_provider == "openvino":
            inference_session_providers.append(
                (
                    facefusion.choices.execution_provider_set.get(execution_provider),
                    {
                        "device_type": resolve_openvino_device_type(
                            execution_device_id
                        ),
                        "precision": "FP32",
                    },
        ...
        execution_devices.append(
            {
                ...
                "product": {
                    "vendor": "NVIDIA",
                    "name": gpu_element.findtext("product_name")
                    .replace("NVIDIA", "")
                    .strip(),
                },
                "video_memory": {
                    "total": create_value_and_unit(
                        gpu_element.findtext("fb_memory_usage/total")
                    ),
                    "free": create_value_and_unit(
                        gpu_element.findtext("fb_memory_usage/free")
                    ),
                },
                "temperature": {
                    "gpu": create_value_and_unit(
                        gpu_element.findtext("temperature/gpu_temp")
                    ),
                    "memory": create_value_and_unit(
                        gpu_element.findtext("temperature/memory_temp")
                    ),
                },
                "utilization": {
                    "gpu": create_value_and_unit(
                        gpu_element.findtext("utilization/gpu_util")
                    ),
                    "memory": create_value_and_unit(
                        gpu_element.findtext("utilization/memory_util")
                    ),
                },
            }
        )
