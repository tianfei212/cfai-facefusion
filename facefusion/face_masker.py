    ...
    return {
            ...
            "hashes": {
                "face_parser": {
                    "url": resolve_download_url(
                        "models-3.1.0", "bisenet_resnet_18.hash"
                    ),
                    "path": resolve_relative_path(
                        "../.assets/models/bisenet_resnet_18.hash"
                    ),
                }
            },
            "sources": {
                "face_parser": {
                    ...
                    "path": resolve_relative_path(
                        "../.assets/models/bisenet_resnet_18.onnx"
                    ),
                }
            },
            "size": (512, 512),
        ...
        "bisenet_resnet_34": {
            "hashes": {
                "face_parser": {
                    "url": resolve_download_url(
                        "models-3.0.0", "bisenet_resnet_34.hash"
                    ),
                    "path": resolve_relative_path(
...
def get_inference_pool() -> InferencePool:
    model_names = [
        state_manager.get_item("face_occluder_model"),
        state_manager.get_item("face_parser_model"),
    ]
...
def collect_model_downloads() -> Tuple[DownloadSet, DownloadSet]:
    ...
    for face_occluder_model in ["xseg_1", "xseg_2", "xseg_3"]:
        if state_manager.get_item("face_occluder_model") in [
            "many",
            face_occluder_model,
        ]:
            model_hash_set[face_occluder_model] = (
                model_set.get(face_occluder_model).get("hashes").get("face_occluder")
            )
            model_source_set[face_occluder_model] = (
                model_set.get(face_occluder_model).get("sources").get("face_occluder")
            )
    ...
    for face_parser_model in ["bisenet_resnet_18", "bisenet_resnet_34"]:
        if state_manager.get_item("face_parser_model") == face_parser_model:
            model_hash_set[face_parser_model] = (
                model_set.get(face_parser_model).get("hashes").get("face_parser")
            )
            model_source_set[face_parser_model] = (
                model_set.get(face_parser_model).get("sources").get("face_parser")
            )
...
def create_box_mask(
    crop_vision_frame: VisionFrame, face_mask_blur: float, face_mask_padding: Padding
) -> Mask:
...
def create_area_mask(
    ...
    for face_mask_area in face_mask_areas:
        if face_mask_area in facefusion.choices.face_mask_area_set:
            landmark_points.extend(
                facefusion.choices.face_mask_area_set.get(face_mask_area)
            )
    ...
    area_mask = (
        cv2.GaussianBlur(area_mask.clip(0, 1), (0, 0), 5).clip(0.5, 1) - 0.5
    ) * 2
    return area_mask
...
def create_region_mask(
    ...
    prepare_vision_frame = cv2.resize(crop_vision_frame, model_size)
    prepare_vision_frame = (
        prepare_vision_frame[:, :, ::-1].astype(numpy.float32) / 255.0
    )
    prepare_vision_frame = numpy.subtract(
        prepare_vision_frame, numpy.array([0.485, 0.456, 0.406]).astype(numpy.float32)
    )
    prepare_vision_frame = numpy.divide(
        prepare_vision_frame, numpy.array([0.229, 0.224, 0.225]).astype(numpy.float32)
    )
...
def forward_occlude_face(prepare_vision_frame: VisionFrame, model_name: str) -> Mask:
    ...
    with conditional_thread_semaphore():
        occlusion_mask: Mask = face_occluder.run(None, {"input": prepare_vision_frame})[
            0
        ][0]
...
def forward_parse_face(prepare_vision_frame: VisionFrame) -> Mask:
    ...
    with conditional_thread_semaphore():
        region_mask: Mask = face_parser.run(None, {"input": prepare_vision_frame})[0][0]
