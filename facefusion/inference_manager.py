...
INFERENCE_POOL_SET: InferencePoolSet = {"cli": {}, "ui": {}}


def get_inference_pool(
    module_name: str, model_names: List[str], model_source_set: DownloadSet
) -> InferencePool:
    ...
    for execution_device_id in execution_device_ids:
        inference_context = get_inference_context(
            module_name, model_names, execution_device_id, execution_providers
        )

        if app_context == "cli" and INFERENCE_POOL_SET.get("ui").get(inference_context):
            INFERENCE_POOL_SET["cli"][inference_context] = INFERENCE_POOL_SET.get(
                "ui"
            ).get(inference_context)
        if app_context == "ui" and INFERENCE_POOL_SET.get("cli").get(inference_context):
            INFERENCE_POOL_SET["ui"][inference_context] = INFERENCE_POOL_SET.get(
                "cli"
            ).get(inference_context)
        if not INFERENCE_POOL_SET.get(app_context).get(inference_context):
            INFERENCE_POOL_SET[app_context][inference_context] = create_inference_pool(
                model_source_set, execution_device_id, execution_providers
            )
...
def create_inference_pool(
    ...
    for model_name in model_source_set.keys():
        ...
        if is_file(model_path):
            inference_pool[model_name] = create_inference_session(
                model_path, execution_device_id, execution_providers
            )
...
def create_inference_session(
    ...
    try:
        inference_session_providers = create_inference_session_providers(
            execution_device_id, execution_providers
        )
        inference_session = InferenceSession(
            model_path, providers=inference_session_providers
        )
        logger.debug(
            wording.get("loading_model_succeeded").format(
                model_name=model_file_name, seconds=calculate_end_time(start_time)
            ),
            __name__,
        )
        ...
        logger.error(
            wording.get("loading_model_failed").format(model_name=model_file_name),
            __name__,
        )
        fatal_exit(1)
...
def get_inference_context(
    ...
    inference_context = ".".join(
        [module_name] + model_names + [execution_device_id] + list(execution_providers)
    )
