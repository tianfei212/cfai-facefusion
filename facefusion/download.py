...
def get_static_download_size(url: str) -> int:
    commands = curl_builder.chain(curl_builder.head(url), curl_builder.set_timeout(5))
...
def conditional_download_hashes(hash_set: DownloadSet) -> bool:
    ...
    if invalid_hash_paths:
        for index in hash_set:
            if hash_set.get(index).get("path") in invalid_hash_paths:
                ...
                if invalid_hash_url:
                    download_directory_path = os.path.dirname(
                        hash_set.get(index).get("path")
                    )
    ...
    for valid_hash_path in valid_hash_paths:
        ...
        logger.debug(
            wording.get("validating_hash_succeeded").format(
                hash_file_name=valid_hash_file_name
            ),
            __name__,
        )
    for invalid_hash_path in invalid_hash_paths:
        ...
        logger.error(
            wording.get("validating_hash_failed").format(
                hash_file_name=invalid_hash_file_name
            ),
            __name__,
        )
...
def conditional_download_sources(source_set: DownloadSet) -> bool:
    source_paths = [
        source_set.get(source_key).get("path") for source_key in source_set.keys()
    ]
    ...
    for valid_source_path in valid_source_paths:
        ...
        logger.debug(
            wording.get("validating_source_succeeded").format(
    ...
    for invalid_source_path in invalid_source_paths:
        ...
        logger.error(
            wording.get("validating_source_failed").format(
                source_file_name=invalid_source_file_name
            ),
            __name__,
        )
        ...
        if remove_file(invalid_source_path):
            logger.error(
                wording.get("deleting_corrupt_source").format(
                    source_file_name=invalid_source_file_name
                ),
                __name__,
            )
...
def resolve_download_url(base_name: str, file_name: str) -> Optional[str]:
    ...
    for download_provider in download_providers:
        download_url = resolve_download_url_by_provider(
            download_provider, base_name, file_name
        )
        if download_url:
            return download_url
...
def resolve_download_url_by_provider(
    download_provider: DownloadProvider, base_name: str, file_name: str
) -> Optional[str]:
    download_provider_value = facefusion.choices.download_provider_set.get(
        download_provider
    )

    for download_provider_url in download_provider_value.get("urls"):
        if ping_static_url(download_provider_url):
            return download_provider_url + download_provider_value.get("path").format(
                base_name=base_name, file_name=file_name
            )
